// components/admin/PackageManagement.jsx
...
const PackageManagement = () => {
  ...
  const fetchPackages = async () => {
    ...
    setSaveError('');
    try {
      const response = await apiCall('/api/SponsorshipPackages', 'GET');
      ...
      setSaveError('Failed to load packages. Please try again.');
  ...
  const fetchPackagesByPriceRange = async () => {
    ...
    try {
      ...
      const response = await apiCall(`/api/SponsorshipPackages/price-range?${params.toString()}`, 'GET');
      ...
      setSaveError('Failed to filter packages. Please try again.');
      setTimeout(() => setSaveError(''), 3000);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSavePackage = async () => {
    ...
    try {
      ...
      if (selectedPackage) {
        ...
        response = await apiCall(`/api/SponsorshipPackages/${selectedPackage.packageId}`, 'PUT', dataToSave);
        setSaveSuccess('✅ Package updated successfully!');
      } else {
        // Create new package
        response = await apiCall('/api/SponsorshipPackages', 'POST', dataToSave);
  ...
  const handleDeletePackage = async () => {
    ...
    try {
      await apiCall(`/api/SponsorshipPackages/${packageToDelete.packageId}`, 'DELETE');
      setSaveSuccess(`✅ "${packageToDelete.packageName}" deleted successfully!`);
  ...
  const handleToggleActive = async (pkg) => {
    try {
      const dataToSave = {
        ...
        price: pkg.price,
        description: pkg.description,
        golfPlayers: pkg.golfPlayers,
        additionalGuests: pkg.additionalGuests,
        brandingCaps: pkg.brandingCaps,
        brandingAllocatedHole: pkg.brandingAllocatedHole,
        brandingShirts: pkg.brandingShirts,
        meetAndGreet: pkg.meetAndGreet,
        golfKitMerchandise: pkg.golfKitMerchandise,
        prizeGivingAcknowledgement: pkg.prizeGivingAcknowledgement,
        websiteAcknowledgement: pkg.websiteAcknowledgement,
        seatWithANCLeadership: pkg.seatWithANCLeadership,
        seatWithCOSATULeadership: pkg.seatWithCOSATULeadership,
        premiumBranding: pkg.premiumBranding,
        isActive: !pkg.isActive,
      };
      
      await apiCall(`/api/SponsorshipPackages/${pkg.packageId}`, 'PUT', dataToSave);
      setSaveSuccess(`✅ Package ${!pkg.isActive ? 'activated' : 'deactivated'} successfully!`);
      await fetchPackages();
      setTimeout(() => setSaveSuccess(''), 3000);
    } catch (error) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2 text-2xl font-bold text-[#132149]">
          <i className="fas fa-box text-[#02a2e0]"></i> Package Management
        </div>
        <button 
      ...
      <div className="card-gradient rounded-3xl p-6 shadow-sm border border-gray-100 mb-6">
        <h4 className="text-sm font-semibold text-[#132149] mb-3">
          <i className="fas fa-sliders-h text-[#02a2e0] mr-2"></i> Filter by Price Range
        </h4>
        <div className="flex flex-wrap items-end gap-3">
          <div>
            <label className="text-sm font-medium text-gray-600 mb-1 block">Min Price (ZAR)</label>
            <input
              type="number"
              name="minPrice"
              ...
              placeholder="0"
              className="w-40 p-3 rounded-2xl border-2 border-gray-200 bg-gray-50 focus:border-[#02a2e0] outline-none"
            />
          </div>
          <div>
            <label className="text-sm font-medium text-gray-600 mb-1 block">Max Price (ZAR)</label>
            <input
              type="number"
              name="maxPrice"
          ...
          <div className="flex gap-2">
            <button
              ...
              className="btn-primary-gradient text-white px-4 py-3 rounded-2xl text-sm font-semibold"
            >
              <i className="fas fa-search mr-1"></i> Apply
            </button>
            <button
              onClick={clearPriceFilter}
              className="border border-gray-300 px-4 py-3 rounded-2xl text-sm font-semibold hover:bg-gray-50"
            >
        ...
        {packages.length === 0 ? (
          ...
          packages.map(pkg => {
            ...
            return (
              <div 
                key={pkg.packageId} 
                className={`card-gradient rounded-2xl p-5 border ${pkg.isActive !== false ? 'border-gray-100' : 'border-red-200 bg-red-50/30'} hover:shadow-md transition-all relative`}
              >
                {pkg.isActive === false && (
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="bg-gray-50 p-2 rounded-xl text-center">
                    ...
                    <span className="font-semibold">{pkg.golfPlayers || 0}</span>
                  </div>
                  <div className="bg-gray-50 p-2 rounded-xl text-center">
                    <span className="text-gray-400 block text-xs">Additional Guests</span>
                    <span className="font-semibold">{pkg.additionalGuests || 0}</span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mt-4 pt-3 border-t border-gray-100">
                  <button 
                    ...
                    className="flex-1 bg-white border border-gray-200 px-3 py-2 rounded-full text-sm font-semibold hover:bg-gray-50 transition-all"
                  >
                    <i className="fas fa-edit text-[#02a2e0] mr-1"></i> Edit
                  </button>
                  <button 
                  ...
                  >
                    <i className={`fas ${pkg.isActive !== false ? 'fa-pause' : 'fa-play'}`}></i>
                  </button>
                  <button 
      ...
      {showDeleteModal && packageToDelete && (
        <Modal onClose={() => { setShowDeleteModal(false); setPackageToDelete(null); }}>
          <div className="text-center max-w-md mx-auto">
            <div className="text-5xl mb-4">🗑️</div>
            <h3 className="text-xl font-bold text-[#132149] mb-2">Confirm Delete</h3>
            <p className="text-gray-500 mb-4">
              Are you sure you want to delete the package <strong>"{packageToDelete.packageName}"</strong>?
            </p>
            ...
            <div className="flex gap-3">
              <button
                ...
                disabled={isSaving}
                className="flex-1 bg-red-500 hover:bg-red-600 text-white px-6 py-3 rounded-full font-semibold flex items-center justify-center gap-2 disabled:opacity-50 transition-colors"
              >
                {isSaving ? (
                  <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> Deleting...</>
                ) : (
                  <><i className="fas fa-trash"></i> Delete Package</>
                )}
              </button>
              <button
                onClick={() => { setShowDeleteModal(false); setPackageToDelete(null); }}
